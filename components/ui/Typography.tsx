...
export function Callout({ children, className = '', track, variant = 'default' }: CalloutProps) {
  const borderColor =
    variant === 'warning'
      ? '#D4A853'
      : variant === 'info'
...
export function Mono({ children, className = '' }: BaseProps) {
  return (
    <span className={`font-mono text-sm text-secondary ${className}`}>{children}</span>
  )
}
...
export function Label({ children, className = '' }: BaseProps) {
  return (
    <span
      className={`font-mono text-xs text-secondary tracking-widest uppercase ${className}`}
    >
      {children}
    </span>
  )
