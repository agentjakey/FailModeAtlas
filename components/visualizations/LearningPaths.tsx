...
export function LearningPaths() {
  ...
  const activePath = LEARNING_PATHS.find((p) => p.id === activePathId) ?? null
      ...
      <div className="flex flex-col gap-3 mb-8">
        {LEARNING_PATHS.map((path, i) => (
          <button
            ...
            type="button"
            onClick={() => {
              setActivePathId(path.id === activePathId ? null : path.id)
              setExpandedSteps(new Set())
            }}
            className="w-full text-left border border-border rounded transition-colors"
            style={{
              background: activePathId === path.id ? '#FFFFFF' : '#F2F0EB',
              padding: '18px 20px',
              borderColor: activePathId === path.id ? '#1A1915' : '#E4E2DB',
            }}
          >
        ...
        >
          ...
          <div style={{ borderTop: '1px solid #E4E2DB', paddingTop: '16px', marginBottom: '20px' }}>
            <p className="font-mono text-[10px] text-accent tracking-widest uppercase mb-4">
              Sequence ({activePath.orderedModeIds.length} steps)
            </p>
            <div className="flex flex-col gap-3">
              {activePath.orderedModeIds.map((modeId, stepIdx) => {
